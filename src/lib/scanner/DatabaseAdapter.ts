...
export class DatabaseAdapter implements IDatabaseAdapter {
  ...
  try {
    ...
    // DEBUG: Log what the inspection returns
    console.log('[DatabaseAdapter] Inspection result:', {
      hasDigestField: 'digest' in inspection,
      hasDigestProperty: inspection.digest !== undefined,
      digestValue: inspection.digest,
      inspectionKeys: Object.keys(inspection),
      inspectionType: typeof inspection
    });
    
    // FIX: Get digest from inspection - handle different formats
    let digest = inspection.digest;
    
    // If digest is undefined, check other possible locations
    if (!digest) {
      console.warn('[DatabaseAdapter] digest is undefined, checking alternative locations...');
      
      // Try different common locations for digest
      if (inspection.Digest) {
        digest = inspection.Digest;
        console.log('[DatabaseAdapter] Found digest in inspection.Digest:', digest);
      } else if (inspection.config?.Digest) {
        digest = inspection.config.Digest;
        console.log('[DatabaseAdapter] Found digest in inspection.config.Digest:', digest);
      } else if (inspection.config?.digest) {
        digest = inspection.config.digest;
        console.log('[DatabaseAdapter] Found digest in inspection.config.digest:', digest);
      } else if (inspection.RepoDigests?.[0]) {
        // Extract digest from RepoDigests (format: "image@sha256:...")
        const repoDigest = inspection.RepoDigests[0];
        const atIndex = repoDigest.indexOf('@');
        if (atIndex !== -1) {
          digest = repoDigest.substring(atIndex + 1);
          console.log('[DatabaseAdapter] Extracted digest from RepoDigests:', digest);
        }
      } else if (inspection.Id) {
        // Use Docker ID as fallback
        digest = inspection.Id;
        console.log('[DatabaseAdapter] Using Docker Id as digest:', digest);
      } else if (inspection.config?.Image) {
        digest = inspection.config.Image;
        console.log('[DatabaseAdapter] Using config.Image as digest:', digest);
      }
      
      // If still no digest, generate a fallback
      if (!digest) {
        console.error('[DatabaseAdapter] No digest found anywhere in inspection!');
        console.error('[DatabaseAdapter] Full inspection:', JSON.stringify(inspection, null, 2));
        
        // Generate a synthetic digest as last resort
        const crypto = require('crypto');
        const uniqueKey = `${cleanImageName}:${request.tag}:${Date.now()}`;
        const hash = crypto.createHash('sha256').update(uniqueKey).digest('hex');
        digest = `sha256:fallback-${hash.substring(0, 32)}`;
        console.warn(`[DatabaseAdapter] Generated fallback digest: ${digest}`);
      }
    } else {
      console.log('[DatabaseAdapter] Using digest from inspection.digest:', digest);
    }
    
    // CRITICAL: Validate digest is defined and not empty
    if (!digest || digest.trim() === '' || digest === 'undefined') {
      throw new Error(`Invalid digest obtained from image inspection: ${digest}. Image: ${cleanImageName}:${request.tag}`);
    }
    ...
    // DEBUG: Log what we're passing to upsert
    console.log('[DatabaseAdapter] Upserting image with digest:', digest);
    console.log('[DatabaseAdapter] Image data:', JSON.stringify(imageData, null, 2));
    ...
    console.log('[DatabaseAdapter] Image upserted successfully:', image.id);
